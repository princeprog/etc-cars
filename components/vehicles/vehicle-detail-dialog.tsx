    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {vehicle.year}
            {vehicle.variant ? ` • ${vehicle.variant}` : ""} • Updated{" "}
        ...
        <div className="grid gap-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <DetailRow
              label="Target price"
              value={vehicle.targetSellingPrice ?? "Not set"}
            />
            <DetailRow
              label="Purchase price"
              value={vehicle.purchasePrice ?? "Not set"}
            />
            <DetailRow
              label="Minimum price"
              value={vehicle.minimumAcceptablePrice ?? "Not set"}
            />
            <DetailRow
              label="Tracked costs"
              value={`PHP ${Number(vehicle.trackedCostsTotal).toLocaleString("en-PH", { minimumFractionDigits: 2 })}`}
            />
            <DetailRow
              label="Mileage"
              value={
            ...
            <DetailRow
              label="Seller lead"
              value={vehicle.sellerLeadId ?? "Direct inventory"}
            />
          </div>
          ...
          <div className="space-y-3">
            ...
            {vehicle.trackedCosts.length ? (
              <div className="space-y-2">
                {vehicle.trackedCosts.map((cost) => (
                  <div
                    key={cost.id}
                    className="flex items-start justify-between gap-4 rounded-md border bg-muted/30 p-3 text-sm"
                  >
                    <div>
                      <p className="font-medium capitalize">{cost.category}</p>
                      <p className="text-muted-foreground">{cost.note}</p>
                    </div>
                    <p className="font-medium tabular-nums">
                      PHP{" "}
                      {Number(cost.amount).toLocaleString("en-PH", {
