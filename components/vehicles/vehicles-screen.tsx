...
type VehiclesScreenProps = {
  initialStatusFilter?: VehicleFilterValue;
};
...
function getStoredVisibleColumns(): VisibleVehicleColumns {
  ...
  try {
    ...
    return {
      stockNumber:
        parsedValue.stockNumber ?? DEFAULT_VISIBLE_VEHICLE_COLUMNS.stockNumber,
      ...
      targetPrice:
        parsedValue.targetPrice ?? DEFAULT_VISIBLE_VEHICLE_COLUMNS.targetPrice,
      minimumPrice:
        parsedValue.minimumPrice ??
        DEFAULT_VISIBLE_VEHICLE_COLUMNS.minimumPrice,
...
export function VehiclesScreen({
  ...
  const activeFilter = parseVehicleStatusFilter(
    statusValues.length > 1 ? statusValues : searchParams.get("status"),
  );
  const [searchTerm, setSearchTerm] = React.useState("");
  const [modelFilter, setModelFilter] = React.useState("all");
  const [qualityFilter, setQualityFilter] =
    React.useState<VehicleQualityFilterValue>("all");
  const [pageState, setPageState] = React.useState({
    statusFilter: initialStatusFilter,
    page: 1,
  });
  const [visibleColumns, setVisibleColumns] =
    React.useState<VisibleVehicleColumns>(getStoredVisibleColumns);
  ...
  const filteredVehicles = filterVehicles(
    ...
    qualityFilter === "all"
      ? true
      : vehicle.qualityScore?.grade === qualityFilter,
  );
  const totalFilteredVehicles = filteredVehicles.length;
  const totalPages = Math.max(
    1,
    Math.ceil(totalFilteredVehicles / VEHICLES_PAGE_SIZE),
  );
  const page = pageState.statusFilter === activeFilter ? pageState.page : 1;
  ...
  const filteredColumnOptions = VEHICLE_COLUMN_MANAGER_OPTIONS.filter(
    (column) =>
      column.label
        .toLowerCase()
        .includes(columnSearchTerm.trim().toLowerCase()),
  );
  ...
  function updateStatusFilter(value: VehicleFilterValue) {
    ...
    setPageState({ statusFilter: value, page: 1 });
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                Manage inventory, review operational readiness, and update
                vehicle details from one table-first workspace.
              </p>
            </div>
            <Button asChild>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-1 flex-col gap-3 md:flex-row md:items-center">
              <div className="relative max-w-sm flex-1">
                ...
                <Input
                  ...
                  onChange={(event) => {
                    ...
                    setPageState({ statusFilter: activeFilter, page: 1 });
                  }}
              ...
              <Select
                value={activeFilter}
                onValueChange={(value) =>
                  updateStatusFilter(value as VehicleFilterValue)
                }
              >
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setPageState({ statusFilter: "all", page: 1 });
                }}
              >
                Reset
              </Button>
            ...
            <div className="flex items-center gap-2 self-end md:self-auto">
              <p className="text-sm text-muted-foreground">
                Showing {paginatedVehicles.length} of {totalFilteredVehicles}{" "}
                filtered vehicles
              </p>
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    size="icon-sm"
                    aria-label="Configure vehicle table columns"
                  >
                    <Settings2Icon />
                  </Button>
                </PopoverTrigger>
                <PopoverContent
                  align="end"
                  className="w-[320px] gap-3 rounded-2xl p-0"
                >
                          ...
                          return (
                            <button
                              ...
                              className={[
                                ...
                                index !== filteredColumnOptions.length - 1
          ...
          totalFilteredVehicles > 0 ? (
            <ListPagination
              ...
              onPageChange={(nextPage) =>
                setPageState({ statusFilter: activeFilter, page: nextPage })
              }
            />
          ) : null}
        </Card>
