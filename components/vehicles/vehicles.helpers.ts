...
export function parseVehicleStatusFilter(
  ...
  return VEHICLE_STATUS_FILTER_VALUES.includes(value as VehicleStatus)
...
export function getVehicleStatusCounts(vehicles: Vehicle[]) {
  return {
    ...
    Available: vehicles.filter((vehicle) => vehicle.status === "Available")
      .length,
    Incoming: vehicles.filter((vehicle) => vehicle.status === "Incoming")
      .length,
    Reconditioning: vehicles.filter(
      (vehicle) => vehicle.status === "Reconditioning",
    ).length,
...
export function getVehicleStatusBadgeVariant(
  status: VehicleStatus,
...
export function previewQualityFromFormValues(
  values: VehicleFormValues,
  options?: {
