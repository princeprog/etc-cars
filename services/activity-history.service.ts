...
export function getActivityHistorySummary(
...
export function getActivityHistory(
  ...
  return apiRequest<ActivityHistoryResponse>(
    API_ENDPOINTS.activityHistory.byEntity(entityType, entityId, limit),
  );
}
...
function buildActivityHistoryQuery(filters?: ActivityHistoryListFilters) {
  ...
  appendOptionalParam(searchParams, "actorUserId", filters?.actorUserId);
